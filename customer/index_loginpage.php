					...
					<div class="bg-body d-flex flex-column align-items-stretch flex-center rounded-4 w-md-600px p-20">
						...
						<div class="d-flex flex-center flex-column flex-column-fluid px-lg-10 pb-15 pb-lg-20">
							...
							<div class="form w-100" novalidate="novalidate" id="kt_sign_in_form" >
								...
								<a href="<?php echo BASE_URL;?>pages/partnerOnBoarding.php" class="link-primary">Sign up</a></div>
								<!--end::Sign up-->
							</div>
							<!--end::Form-->
						</div>
		...
		$("#kt_sign_in_submit").on("click",function(e){
			...
			$.ajax({
				...
				success: function(resultData) {
					...
					}else if(jsonObj.status=="success" && jsonObj.data.userdata.new_pwd==1){
						var userdata = jsonObj.data.userdata;
						localStorage.setItem('email',userdata.email);
						Swal.fire({
							text: "Temperory Password Matched! \n You will be redirected to set new password screen.",
							icon: "success",
							buttonsStyling: !1,
							confirmButtonText: "Ok, got it!",
							customClass: {
							...
							if (e.isConfirmed) {
								$(".indicator-label").css("display","block");
								$(".indicator-progress").css("display","none");
								location.href = "<?php echo BASE_URL;?>newPassword.php";
							}
						}))
					}else{
						$(".indicator-label").css("display","block");
